...
function SenStatsDashboardContent({ darkMode }: { darkMode: boolean }) {
  ...
  return (
    <section className={`card senstats-dashboard ${darkMode ? 'senstats-dark' : ''}`}>
      <div className="senstats-tabs senstats-main-menu" role="tablist" aria-label="SenStats sections">
        {([
          { key: 'senators', icon: '👥', label: 'Senators' },
          { key: 'dashboards', icon: '📊', label: 'Dashboards' },
          { key: 'committees', icon: '🏛️', label: 'Committees' },
          { key: 'sources', icon: '🔎', label: 'Data sources' },
          { key: 'changes', icon: '🧾', label: 'Change log' },
        ] as Array<{ key: SenStatsTab; icon: string; label: string }>).map((tab) => <button key={tab.key} type="button" className={activeTab === tab.key ? 'active' : ''} onClick={() => setActiveTab(tab.key)}><span aria-hidden="true">{tab.icon}</span>{tab.label}</button>)}
      </div>
