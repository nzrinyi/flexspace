...
function ExpenseBarChart({ rows, emptyLabel = 'No expense records available for this view.', valueLabel = 'Total', onSelectRow }: { rows: ExpenseAggregateRow[]; emptyLabel?: string; valueLabel?: string; onSelectRow?: (key: string) => void }) {
  ...
  return <div className="expense-pivot-chart" role="img" aria-label={`Expense ${valueLabel.toLowerCase()} by selected pivot`}>
    {rows.map((row) => {
      const content = <><div><strong>{row.label}</strong><span>{isPerSenator ? `${row.senatorCount} senators · total ${currency(row.total)}` : `${row.count} records · average per record ${currency(row.average)}`}</span></div><div className="expense-pivot-track"><i style={{ width: `${Math.max((row.value / maxValue) * 100, 3)}%` }} /></div><b data-label={valueLabel}><span>{valueLabel}</span>{currency(row.value)}</b></>;
      return onSelectRow ? <button type="button" key={row.key} className={`expense-pivot-row ${row.className || ''}`} title={`${row.label}: ${valueLabel} ${currency(row.value)}`} onClick={() => onSelectRow(row.key)}>{content}</button> : <article key={row.key} className={`expense-pivot-row ${row.className || ''}`} title={`${row.label}: ${valueLabel} ${currency(row.value)}`}>{content}</article>;
    })}
  </div>;
}
...
export function SenStatsExpensesDashboardView({ senators, expenses, selectedSenator, onSelectSenator, loading }: { senators: SenStatsSenatorDocument[]; expenses: ExpenseRow[]; selectedSenator?: SenStatsSenatorDocument; onSelectSenator: (id: string) => void; loading: boolean }) {
  ...
  const [selectedExpenseSenatorKey, setSelectedExpenseSenatorKey] = useState('');
  ...
  const selectedExpenseSenator = selectedExpenseSenatorKey ? senatorsById.get(selectedExpenseSenatorKey) : selectedSenator;
  const selectedExpenseLabel = selectedExpenseSenator?.name || senatorExpenseRows.find((row) => row.key === selectedExpenseSenatorKey)?.label || selectedSenator?.name || '';
  const selectedSenatorExpenses = useMemo(() => {
    const key = selectedExpenseSenator?.id || selectedExpenseSenatorKey || selectedSenator?.id || '';
    if (!key) return [];
    return yearFilteredExpenses.filter((expense) => expense.senatorId === key || (!expense.senatorId && expense.senatorName === key));
  }, [yearFilteredExpenses, selectedExpenseSenator, selectedExpenseSenatorKey, selectedSenator]);
  ...
  return <section className="source-card source-wide expenses-dashboard" aria-label="Expense dashboards">
    ...
    {activePivot === 'groups' && <div className="expenses-dashboard-layout"><ExpensePivotSelector rows={groupRows} activeKey={selectedGroup} onSelect={setSelectedGroup} label="Group selector" /><div className="expense-pivot-content"><div className="source-heading"><span>Average per senator</span><strong>{selectedGroup === 'All' ? 'All parliamentary groups' : groupLabel(selectedGroup)}</strong><small>Groups are compared by average spending per senator; totals are shown as context.</small></div><ExpenseBarChart valueLabel={selectedGroup === 'All' ? 'Average per senator' : 'Total'} rows={selectedGroup === 'All' ? groupRows : aggregateExpenses(filteredByGroup, (expense) => { const senator = senatorsById.get(expense.senatorId || ''); return { key: senator?.id || expense.senatorId || expense.senatorName || 'Unknown', label: senator?.name || expense.senatorName || 'Unknown senator', className: groupClassName(senator?.party || '') }; })} onSelectRow={selectedGroup === 'All' ? setSelectedGroup : setSelectedExpenseSenatorKey} /></div></div>}

    {activePivot === 'provinces' && <div className="expenses-dashboard-layout"><ExpensePivotSelector rows={provinceRows} activeKey={selectedProvince} onSelect={setSelectedProvince} label="Province selector" /><div className="expense-pivot-content"><div className="source-heading"><span>Average per senator</span><strong>{selectedProvince === 'All' ? 'All provinces and territories' : selectedProvince}</strong><small>Provinces are compared by average spending per senator; totals are shown as context.</small></div><ExpenseBarChart valueLabel={selectedProvince === 'All' ? 'Average per senator' : 'Total'} rows={selectedProvince === 'All' ? provinceRows : aggregateExpenses(filteredByProvince, (expense) => { const senator = senatorsById.get(expense.senatorId || ''); return { key: senator?.id || expense.senatorId || expense.senatorName || 'Unknown', label: senator?.name || expense.senatorName || 'Unknown senator', className: groupClassName(senator?.party || '') }; })} onSelectRow={selectedProvince === 'All' ? setSelectedProvince : setSelectedExpenseSenatorKey} /></div></div>}

    {activePivot === 'individuals' && <div className="expenses-dashboard-layout"><ExpensePivotSelector rows={senatorExpenseRows} activeKey={selectedExpenseSenatorKey || selectedSenator?.id || 'All'} onSelect={(key) => setSelectedExpenseSenatorKey(key === 'All' ? '' : key)} label="Senator selector" /><div className="expense-pivot-content"><div className="source-heading expense-detail-heading"><div><span>Individual deep dive</span><strong>{selectedExpenseLabel || 'Select a senator'}</strong><small>{selectedSenatorExpenses.length ? `${selectedSenatorExpenses.length} records · ${currency(selectedSenatorExpenses.reduce((sum, expense) => sum + (Number(expense.amount) || 0), 0))}` : 'No expense records available for this senator.'}</small></div>{selectedExpenseSenator && <button type="button" className="expense-profile-button" onClick={() => onSelectSenator(selectedExpenseSenator.id)}>View senator profile</button>}</div>{selectedExpenseLabel ? <><SenStatsExpenseChart expenses={selectedSenatorExpenses} loading={false} /><div className="dashboard-table expense-quarter-table" role="table" aria-label="Selected senator quarterly expense totals"><div role="row" className="source-table-head"><span>Quarter</span><span>Total</span><span>Categories</span></div>{selectedQuarterRows.length === 0 ? <div role="row"><strong>No records available</strong><span>—</span><span>Quarterly expense rows will appear after the sync finds this senator.</span></div> : selectedQuarterRows.map((row) => { const total = Object.entries(row).filter(([key]) => key !== 'quarter').reduce((sum, [, value]) => sum + Number(value || 0), 0); const categories = Object.keys(row).filter((key) => key !== 'quarter').join(', '); return <div role="row" key={String(row.quarter)}><strong>{String(row.quarter)}</strong><span data-label="Total">{currency(total)}</span><span data-label="Categories">{categories || 'Other'}</span></div>; })}</div></> : <p className="muted">Choose a senator from the selector to see the individual quarterly breakdown. This opens the expense details here; use the profile button only when you want the full senator drawer.</p>}</div></div>}
  </section>;
}
