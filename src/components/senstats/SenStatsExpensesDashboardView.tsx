...
function aggregateExpenses(expenses: ExpenseRow[], keyForExpense: (expense: ExpenseRow) => { key: string; label?: string; className?: string }) {
  const rows = new Map<string, ExpenseAggregateRow>();
  expenses.forEach((expense) => {
    const amount = Number(expense.amount) || 0;
    const { key, label, className } = keyForExpense(expense);
    const current = rows.get(key) ?? { key, label: label || key, total: 0, count: 0, average: 0, className };
...
function ExpenseBarChart({ rows, emptyLabel = 'No expense records available for this view.' }: { rows: ExpenseAggregateRow[]; emptyLabel?: string }) {
  ...
  return <div className="expense-pivot-chart" role="img" aria-label="Expense totals by selected pivot">
    {rows.map((row) => <article key={row.key} className={`expense-pivot-row ${row.className || ''}`} title={`${row.label}: ${currency(row.total)}`}>
      <div><strong>{row.label}</strong><span>{row.count} records · avg. {currency(row.average)}</span></div>
      <div className="expense-pivot-track"><i style={{ width: `${Math.max((row.total / maxTotal) * 100, 3)}%` }} /></div>
...
function ExpensePivotSelector({ rows, activeKey, onSelect, label }: { rows: ExpenseAggregateRow[]; activeKey: string; onSelect: (key: string) => void; label: string }) {
  return <aside className="expense-pivot-selector" aria-label={label}>
    ...
    {rows.map((row) => <button key={row.key} type="button" className={`${activeKey === row.key ? 'active ' : ''}${row.className || ''}`} onClick={() => onSelect(row.key)}><strong>{row.label}</strong><small>{currency(row.total)}</small></button>)}
  </aside>;
}
...
export function SenStatsExpensesDashboardView({ senators, expenses, selectedSenator, onSelectSenator, loading }: { senators: SenStatsSenatorDocument[]; expenses: ExpenseRow[]; selectedSenator?: SenStatsSenatorDocument; onSelectSenator: (id: string) => void; loading: boolean }) {
  ...
  return <section className="source-card source-wide expenses-dashboard" aria-label="Expense dashboards">
    ...
    {activePivot === 'groups' && <div className="expenses-dashboard-layout"><ExpensePivotSelector rows={groupRows} activeKey={selectedGroup} onSelect={setSelectedGroup} label="Group selector" /><div className="expense-pivot-content"><div className="source-heading"><span>Group totals</span><strong>{selectedGroup === 'All' ? 'All parliamentary groups' : groupLabel(selectedGroup)}</strong><small>Comparative spending by parliamentary group.</small></div><ExpenseBarChart rows={selectedGroup === 'All' ? groupRows : aggregateExpenses(filteredByGroup, (expense) => { const senator = senatorsById.get(expense.senatorId || ''); return { key: senator?.name || expense.senatorName || 'Unknown', label: senator?.name || expense.senatorName || 'Unknown senator', className: groupClassName(senator?.party || '') }; })} /></div></div>}

    {activePivot === 'provinces' && <div className="expenses-dashboard-layout"><ExpensePivotSelector rows={provinceRows} activeKey={selectedProvince} onSelect={setSelectedProvince} label="Province selector" /><div className="expense-pivot-content"><div className="source-heading"><span>Province totals</span><strong>{selectedProvince === 'All' ? 'All provinces and territories' : selectedProvince}</strong><small>Total and average spending by province/territory.</small></div><ExpenseBarChart rows={selectedProvince === 'All' ? provinceRows : aggregateExpenses(filteredByProvince, (expense) => { const senator = senatorsById.get(expense.senatorId || ''); return { key: senator?.name || expense.senatorName || 'Unknown', label: senator?.name || expense.senatorName || 'Unknown senator', className: groupClassName(senator?.party || '') }; })} /></div></div>}
