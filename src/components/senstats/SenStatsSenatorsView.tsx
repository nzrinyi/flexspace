    ...
    <div className="senstats-grid">
      <aside className="senator-list" aria-label="Canadian senators">
        ...
        {filteredSenators.map((senator) => {
          ...
          return <button key={senator.id} type="button" className={senator.id === selectedSenator?.id ? `active ${groupClassName(senator.party)}` : groupClassName(senator.party)} onClick={() => onSelectSenator(senator.id)}><span className="senator-avatar">{photoUrl && <img src={photoUrl} alt="" referrerPolicy="no-referrer" onError={(event) => { event.currentTarget.hidden = true; }} />}<i aria-hidden="true">{senator.name.slice(0, 1)}</i></span><strong>{senator.name}{recentlyChangedSenatorIds.has(senator.id) && <span className="history-badge" title="Recent affiliation change">↻</span>}</strong><span>{senator.province}</span><em>{senator.party}</em></button>;
        })}
      </aside>
