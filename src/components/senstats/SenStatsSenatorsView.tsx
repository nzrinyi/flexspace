  ...
  return <>
    ...
    <div className="senstats-grid">
      <aside className="senator-list" aria-label="Canadian senators">
        ...
        {filteredSenators.map((senator) => <button key={senator.id} type="button" className={senator.id === selectedSenator?.id ? `active ${groupClassName(senator.party)}` : groupClassName(senator.party)} onClick={() => onSelectSenator(senator.id)}>{senator.photoUrl ? <img src={senator.photoUrl} alt="" /> : <i aria-hidden="true">{senator.name.slice(0, 1)}</i>}<strong>{senator.name}{recentlyChangedSenatorIds.has(senator.id) && <span className="history-badge" title="Recent affiliation change">↻</span>}</strong><span>{senator.province}</span><em>{senator.party}</em></button>)}
      </aside>
      <div className="expense-panel">
        {selectedSenator ? <><div className={`senstats-stat selected-senator-card ${groupClassName(selectedSenator.party)}`}>{selectedSenator.photoUrl && <img src={selectedSenator.photoUrl} alt={`${selectedSenator.name} portrait`} />}<span>Selected senator</span><strong>{selectedSenator.name}</strong><small>{selectedSenator.province} · {selectedSenator.party}</small>{selectedSenator.sourceUrl && <a href={selectedSenator.sourceUrl} target="_blank" rel="noreferrer">Official profile</a>}</div><div className="senstats-stat"><span>Visible expenses</span><strong>{expenses.length ? currency(totalExpenses) : 'No data available'}</strong><small>{expenseLoading ? 'Syncing expense records…' : `${expenses.length} quarterly records`}</small></div>{selectedProfileFields.length > 0 && <div className="senstats-profile-data"><strong>Additional profile data</strong>{selectedProfileFields.map(([key, value]) => <p key={key}><span>{key.replace(/-/g, ' ')}</span><small>{String(value)}</small></p>)}</div>}<SenStatsExpenseChart expenses={expenses} loading={expenseLoading} /></> : <p className="muted">Choose a senator to inspect expenses.</p>}
      </div>
    </div>
  </>;
}
