...
export function SenStatsChangeLogView({ changes, onSelectSenator }: { changes: Array<SenStatsChangeLogDocument & { id: string }>; onSelectSenator: (id: string) => void }) {
  ...
  return <section className="change-log-list" aria-label="SenStats change log">
    {changes.map((change) => <article key={change.id} className={`change-log-item ${change.type}`}>
      <div><span>{change.type.replace(/_/g, ' ')}</span><button type="button" className="change-log-senator" onClick={() => onSelectSenator(change.senatorId)}>{change.senatorName}</button><small>{formatDate(change.detectedAt)}</small></div>
