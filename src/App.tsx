...
function ArtSizerApp({ onOpenAppSelection }: { onOpenAppSelection: () => void }) {
  ...
  const [moveModePieceId, setMoveModePieceId] = useState<string | null>(null);
  ...
  const beginFramePointer = useCallback((event: PointerEvent<SVGGElement>, piece: WallArtPiece) => {
    ...
    const isMoveModeActive = moveModePieceId === piece.id;
    if (isMoveModeActive) event.preventDefault();
    event.currentTarget.setPointerCapture(event.pointerId);
    setSelectedPieceId(piece.id);

    if ((event.pointerType === 'touch' || event.pointerType === 'pen') && !isMoveModeActive) {
    ...
    setDraggingPiece(nextDrag);
  }, [canvasPoint, clearLongPressTimer, moveModePieceId]);
  ...
  const deletePiece = useCallback((pieceId: string) => {
    ...
    setMoveModePieceId((current) => current === pieceId ? null : current);
  }, [setPieces]);
  const rotatePiece = useCallback((pieceId: string) => {
    setPieces((current) => current.map((piece) => piece.id === pieceId ? { ...piece, rotation: ((piece.rotation ?? 0) + 90) % 360 } : piece));
    setSelectedPieceId(pieceId);
  }, [setPieces]);
  const nudgePiece = useCallback((pieceId: string, deltaX: number, deltaY: number) => {
    const step = snapEnabled ? 2 : 1;
    setPieces((current) => current.map((piece) => {
      if (piece.id !== pieceId) return piece;
      const size = renderedArtSize(piece, scalePercent);
      return clampArtPiece({ ...piece, x: snapValue(piece.x + deltaX * step, snapEnabled), y: snapValue(piece.y + deltaY * step, snapEnabled) }, wallWidth, wallHeight, size.width, size.height);
    }));
    setSelectedPieceId(pieceId);
  }, [scalePercent, setPieces, snapEnabled, wallHeight, wallWidth]);

  const resizePiece = useCallback((pieceId: string, direction: 1 | -1) => {
    setPieces((current) => current.map((piece) => {
      if (piece.id !== pieceId) return piece;
      const currentIndex = Math.max(standardArtSizes.findIndex((size) => size.label === piece.label), 0);
      const nextSize = standardArtSizes[Math.min(Math.max(currentIndex + direction, 0), standardArtSizes.length - 1)];
      return { ...piece, label: nextSize.label, width: nextSize.width, height: nextSize.height, color: nextSize.color };
    }));
    setSelectedPieceId(pieceId);
  }, [setPieces]);
      ...
      <section className="card artsizer-app">
        ...
        <div className="artsizer-layout">
          <aside className="artsizer-drawer" aria-label="Standard frame sizes">
            ...
            <div className="artsizer-actions"><button type="button" onClick={() => setSnapEnabled((current) => !current)}>{snapEnabled ? 'Turn snapping off' : 'Turn snapping on'}</button><button type="button" onClick={() => setShowSpacing((current) => !current)}>{showSpacing ? 'Hide spacing' : 'Show spacing'}</button><button type="button" onClick={resetLayout}>Reset</button></div>{selectedPiece && <div className="artsizer-selection"><p>Selected: <strong>{selectedPiece.label}</strong>. Use arrows to move by {snapEnabled ? '2″ snap units' : '1″ steps'}.</p><div className="artsizer-nudge-pad" aria-label="Move selected frame"><span>Move</span><button type="button" aria-label="Move frame up" onClick={() => nudgePiece(selectedPiece.id, 0, -1)}>↑</button><button type="button" aria-label="Move frame left" onClick={() => nudgePiece(selectedPiece.id, -1, 0)}>←</button><button type="button" aria-label="Move frame right" onClick={() => nudgePiece(selectedPiece.id, 1, 0)}>→</button><button type="button" aria-label="Move frame down" onClick={() => nudgePiece(selectedPiece.id, 0, 1)}>↓</button></div><div className="artsizer-selected-actions" aria-label="Selected frame actions"><button type="button" onClick={() => resizePiece(selectedPiece.id, 1)}>↑ Upsize</button><button type="button" onClick={() => resizePiece(selectedPiece.id, -1)}>↓ Downsize</button><button type="button" onClick={() => rotatePiece(selectedPiece.id)}>↻ Rotate</button><button type="button" className="danger" onClick={() => deletePiece(selectedPiece.id)}>× Delete</button></div></div>}
          ...
          <div className="wall-stage">
            <div className="wall-canvas-viewport"><div className="wall-canvas-zoom" style={{ width: `${zoomPercent}%` }}><svg className={`wall-canvas ${moveModePieceId ? 'move-mode' : ''}`} viewBox={`0 0 ${wallWidth} ${wallHeight}`} role="img" aria-label="Wall canvas for arranging framed art" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const label = event.dataTransfer.getData('application/artsizer-frame'); const size = standardArtSizes.find((option) => option.label === label); if (!size || !(event.currentTarget instanceof SVGSVGElement)) return; const point = canvasPoint(event.clientX, event.clientY, event.currentTarget); addFrame(size, point.x - size.width / 2, point.y - size.height / 2); }} onPointerMove={(event) => { if (!draggingPiece || !(event.currentTarget instanceof SVGSVGElement)) return; const point = canvasPoint(event.clientX, event.clientY, event.currentTarget); setPieces((current) => current.map((piece) => { if (piece.id !== draggingPiece.id) return piece; const size = renderedArtSize(piece, scalePercent); return clampArtPiece({ ...piece, x: snapValue(point.x - draggingPiece.offsetX, snapEnabled), y: snapValue(point.y - draggingPiece.offsetY, snapEnabled) }, wallWidth, wallHeight, size.width, size.height); })); }} onPointerUp={endFramePointer} onPointerCancel={endFramePointer} onPointerLeave={endFramePointer}>
              ...
              {effectivePieces.map((piece) => {
                ...
                const controlButtons = [
                  { label: 'Upsize frame', icon: '↑', x: controlX, y: controlY, onClick: () => resizePiece(piece.id, 1) },
                  { label: 'Downsize frame', icon: '↓', x: controlX + 4, y: controlY, onClick: () => resizePiece(piece.id, -1) },
            ...
            {selectedPiece && <div className="artsizer-canvas-actions" aria-label="Quick frame movement controls"><strong>{selectedPiece.label}</strong><button type="button" aria-label="Move frame up" onClick={() => nudgePiece(selectedPiece.id, 0, -1)}>↑</button><button type="button" aria-label="Move frame left" onClick={() => nudgePiece(selectedPiece.id, -1, 0)}>←</button><button type="button" aria-label="Move frame right" onClick={() => nudgePiece(selectedPiece.id, 1, 0)}>→</button><button type="button" aria-label="Move frame down" onClick={() => nudgePiece(selectedPiece.id, 0, 1)}>↓</button><button type="button" aria-label="Rotate frame" onClick={() => rotatePiece(selectedPiece.id)}>↻</button><button type="button" className="danger" aria-label="Delete frame" onClick={() => deletePiece(selectedPiece.id)}>×</button></div>}
            <p className="artsizer-help">Tip: tap a frame, then use the arrow pad to move it by snap units on mobile. Canvas zoom changes your view only — the wall dimensions stay fixed.</p>
          </div>
        </div>
      </section>
    </main>
...
function AuthenticatedSession({ activeUser }: { activeUser: User }) {
  ...
  const [activeWorkspaceApp, setActiveWorkspaceApp] = useState<WorkspaceApp>('AppSelection');
